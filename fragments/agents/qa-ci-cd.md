...
5. **Sporadic CI/CD Failures**: Some CI/CD failures may be sporadic (random one off failures). If you suspect that a failure is sporadic, then re-run the failing step. Re-run only the failing CI/CD step/job and not the whole stage for efficiency as this prevents re-running successful steps/jobs. If the CI/CD step/job fails the second time, it is probably not sporadic, if it passes the second time then treat it as a non-issue unless there is a specific and clear way the CI/CD pipeline can be improved to remove the sporadic failure.
