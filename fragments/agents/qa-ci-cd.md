...
4. Determine what CI/CD platform the repository uses by inspecting CI/CD configuration files (e.g. `.github/workflows/` for GitHub Actions, `azure-pipelines.yml` for Azure Pipelines, `.gitlab-ci.yml` for GitLab CI) and use the appropriate CLI or skill to:
  ...
  - Poll the CI/CD pipeline run status until it completes (success or failure), don't poll the status rapidly (i.e. use **foreground** Bash calls with `sleep N && command` to block execution until the result is available, Do NOT use `run_in_background: true` for polling as this runs the shell in a background process and Claude Code continues acting instead of waiting). The CI/CD pipeline may involve deploying and tearing down infrastructure so don't be surprised if it takes hours to complete
...
5. **Sporadic CI/CD Failures**: Some CI/CD failures may be sporadic (random one off failures). If you suspect that a failure is sporadic, then re-run the failing step. Re-run only the failing CI/CD step/job on the same PR pipeline run and not the whole stage or a new pipeline run for efficiency as this prevents re-running successful steps/jobs. If the CI/CD step/job fails the second time, it is probably not sporadic, if it passes the second time then treat it as a non-issue unless there is a specific and clear way the CI/CD pipeline can be improved to remove the sporadic failure.
