...
- **Sub-agent spawn restrictions**: Leaf agents that never call other agents (`tr-code-review`, `tr-qa-tester`, `tr-qa-ci-cd`) declare `disallowedTools: Agent` in their fragment frontmatter so they cannot spawn sub-agents; guarded by `TestSubagentSpawnRestrictions` in `tests/test_compose.py`.
